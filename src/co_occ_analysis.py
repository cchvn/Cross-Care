...
def find_co_occurrences(text, medical_patterns, racial_patterns, gender_patterns, drug_patterns, window_size):
    """
    Find co-occurrences of terms in a given text within a specified window size.
    """
    co_occurrences = {'racial': defaultdict(int), 'gender': defaultdict(int), 'drug': defaultdict(int)}
    ...
    for med_key, med_pattern in medical_patterns.items():
        for med_match in med_pattern.finditer(text):
            ...
            for category, patterns in [('racial', racial_patterns), ('gender', gender_patterns), ('drug', drug_patterns)]:
...
def process_texts_in_parallel(data, medical_dict, racial_dict, gender_dict, drug_dict, window_size):
    """
    ...
    with Pool(cpu_count()) as pool:
        results = list(tqdm(pool.imap(partial(find_co_occurrences, 
    ...
    # Aggregate results
    aggregated_results = {'racial': defaultdict(int), 'gender': defaultdict(int), 'drug': defaultdict(int)}
...
def analyze_data_co_occurrence(source_name, data_path, medical_dict, racial_dict, gender_dict, drug_dict):
    ...
    disease_date_counts = df_output.groupby(grouping_column)[list(medical_dict.keys())].sum().reset_index()
    ...
    for window in window_sizes:
        aggregated_results = process_texts_in_parallel(data, medical_dict, racial_dict, gender_dict, drug_dict, window)
